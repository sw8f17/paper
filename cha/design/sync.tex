...
\subsubsection{Light Cues}\tnote{I think we should remove light cues all together. How is it realistic in relation to Android?}
...
\section{Refining the Synchronization Method --- SNTP}
...
\begin{description}
    ...
    While effective in real-time systems, where the device clock needs to be synchronized, it has no impact on our application due to us not sending real-time commands, but rather timestamps for when an event should occur.\tnote{I don't understand this.}
    ...
    Similarly to how drift alleviation works, the same technique is used by \ac{NTP} for when the clock is only slightly out of sync, this way a step in time is not introduced, which is how \ac{SNTP} synchronizes the clock.\tnnote{Jeg forstår ikke det, især det med ``step''}
