...
Other services such as Twitch\footnote{\url{https://www.twitch.tv/}} and YouTube\footnote{\url{https://www.youtube.com/}} use allow for both livestreaming, and streaming of pre--recorded content.
...
The apps described in \cref{sec:sota_apps} provides insight into what such an app should be capable of, for each feature we can consider the reasoning behind adding that feature and therefrom determine possible problems we will need to address in our app.
...
The two apps use their own respective solutions for device connectivity, SoundSeeder uses WiFi/android hotspot where AmpMe simply uses an internet connection.
This implies that either solution is capable of solving the problem, tests in \cref{}\fxnote{ref til testafsnit} evaluate just how good these solutions are and determines whether one solution performs better than the other.
...
SoundSeeder allows the user to change at intervals of $10ms$ between $-400ms$ and $+400ms$ whereas AmpMe uses an arbitrary value from $-15$ to $+15$.
...
Our insight into video streaming may help shed some light on as to why proprietary software is used rather than the more common streaming protocols, in this field the companies use a variety of well established protocols rather than proprietary ones.
...
Knowing that some audio solutions use \ac{RTP} and AirPlay is based on \ac{RTSP}, these two protocols in particular may be worth consideration.
