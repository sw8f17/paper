    ...
    \resizebox{\linewidth}{!}{%
        \begin{tikzpicture}
            \begin{axis}[legend pos=north west,
                ...
                \addplot table [col sep=comma, only marks] {\relativepath ampme_long_norm.csv};
                %\addplot[domain=-15:15, samples=100,red]{(23.211)*x+131.26};
                %\legend{$Data$,$f(x)=23.211*x+131.26$}
            \end{axis}
        \end{tikzpicture}
    }
    \caption{AmpMe}\label{fig:ampme_drift_test}
\end{subfigure}%
\begin{subfigure}{.5\linewidth}
    \centering
    \resizebox{\linewidth}{!}{%
        \begin{tikzpicture}
            \begin{axis}[legend pos=north west,
                ...
                xlabel={Time in seconds},
                ylabel={Actual Delay in ms}]
                \addplot table [col sep=comma, only marks] {\relativepath ss_long_norm.csv};
                %\addplot[domain=-15:15, samples=100,red]{(23.211)*x+131.26};
                %\legend{$Data$,$f(x)=23.211*x+131.26$}
            \end{axis}
        \end{tikzpicture}
    }
    \caption{SoundSeeder}\label{fig:ss_drift_test}
\end{subfigure}%
    \caption{Drift test results.}%
