...
Only its connectivity options seems on par with the other two mentioned apps, however reviews mention that it is increadibly slow, as such we see no reason to continue with this app, nor can we deem it state of the art.
...
As for connectivty, SoundSeeder supports streaming via WiFi or Android hotspot.
...
As for the presented use cases in \cnameref{cha:establishing_use_cases}, either solution is acceptable.
...
The need for manual synchronization options could indicate that the automatic synchronization can at times be faulty, yet it is preferable to reconnecting.
