...
To explain how the service discovery occurs we have constructed an slightly simplified UML sequence diagram, on \cref{fig:seq_server_client}.
...
After accepting a connection it creates a MessageConnection, which is our abstraction, which is run on a separate thread for each connection from the server to the client, and vice versa.
...
To solve these issue, we add two bytes to each message we send.
