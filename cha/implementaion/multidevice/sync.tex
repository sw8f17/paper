...
This method would not synchronize each client to a global clock, like \ac{NTP} and \ac{SNTP}, but rather synchronize the clock of the clients to the clock of the server\tnnote{Er det ment son en generel ``server'', eller skal det være ``master''?}.
...
Then we will benchmark them to determine the solution which requires internet is better, and thusly justify the added requirement for the users\tnwarning{We need to do this}.
...
This is illustrated in \cref{fig:ntp_packets}\tnnote{Måske pile på figuren, så man ved at T0 går til T1 og T2 til T3}.
